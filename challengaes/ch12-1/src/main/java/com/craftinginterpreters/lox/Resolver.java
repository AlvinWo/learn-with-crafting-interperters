...
public class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  public Void visitClassStmt(Stmt.Class stmt) {
    ...
    for (Stmt.Function function : stmt.methods) {
      FunctionType declaration =
          function.name.lexeme.equals("init") ? FunctionType.INITIALIZER : FunctionType.METHOD;
      resolveFunction(function, declaration);
    }

    for (Stmt.Function staticFunction : stmt.staticMethods) {
      beginScope();
      // I didn't support this here; this is from the official answer
      scopes.peek().put("this", true);
  ...
  public Void visitWhileStmt(Stmt.While stmt) {
    resolve(stmt.condition);
    // TODO why we don't need a scope
    resolve(stmt.body);
    return null;
  }
